...
public class SqlcipherPlugin extends ReactContextBaseJavaModule {
    ...
    public static final String TAG = SqlcipherPlugin.class.getSimpleName();
    ...
    public SqlcipherPlugin(ReactApplicationContext reactContext) {
        super(reactContext);
    ...
    private boolean executeAndPossiblyThrow(Action action, ReadableMap args, CallbackContext cbc) throws Exception {
        String dbname;

        switch (action) {
            case echoStringValue:
                String echo_value = SqlcipherPluginConverter.getString(args,"value","");
            ...
            case attach:
                dbname = SqlcipherPluginConverter.getString(args,"path","");
                String dbAlias = SqlcipherPluginConverter.getString(args,"dbAlias","");
                String dbNameToAttach = SqlcipherPluginConverter.getString(args,"dbName","");
            ...
            case backgroundExecuteSqlBatch:
                ...
                ReadableMap dbArgs = (ReadableMap) SqlcipherPluginConverter.get(args,"dbargs",null);
                dbname = SqlcipherPluginConverter.getString(dbArgs,"dbname","");
                ReadableArray txArgs = (ReadableArray) SqlcipherPluginConverter.get(args,"executes",null);
                ...
                if (txArgs.isNull(0)) {
                    ...
                    for (int i = 0; i < len; i++) {
                        ReadableMap queryArgs = (ReadableMap) SqlcipherPluginConverter.get(txArgs,i,null);
                        queries[i] = SqlcipherPluginConverter.getString(queryArgs,"sql","");
                        queryIDs[i] = SqlcipherPluginConverter.getString(queryArgs,"qid","");
                        queryParams[i] = (ReadableArray) SqlcipherPluginConverter.get(queryArgs,"params",null);
                    }
                }
     ...
     *
     * @param dbname dbname - The name of the database file
     * @param assetFilePath - path to the pre-populated database file
     * @param openFlags - the db open options
     * @param isForced  - Forced to cover db file
     */
    protected File openDbFile(String dbname,String assetFilePath,int openFlags,boolean isForced) throws Exception {
        InputStream in = null;
        File dbfile = null;
        try {
            ...
            if (dbfile == null) {
                ...
                if(isForced && dbfile.exists()) dbfile.delete();
    ...
    private void bindArgsToStatement(SQLiteStatement myStatement, ReadableArray sqlArgs) {
        for (int i = 0; i < sqlArgs.size(); i++) {
            ...
            if (type == ReadableType.Number){
                ...
                myStatement.bindString(i + 1, SqlcipherPluginConverter.getString(sqlArgs,i,""));
            }
        }
    }
                ...
                if (queryParams != null) {
                    ...
                    for (int j = 0; j < size; j++) {
                        if (queryParams.isNull(j)) {
                            ...
                            params[j] = SqlcipherPluginConverter.getString(queryParams, j, "");
                        }
                    }
                }
        ...
        DBRunner(final String dbname, ReadableMap options, CallbackContext cbc) {
            ...
            try {
                ...
                if (this.assetFilename != null && this.assetFilename.length() > 0) {
                    boolean readOnly = SqlcipherPluginConverter.getBoolean(options,"readOnly",false);
            ...
            this.androidLockWorkaround = SqlcipherPluginConverter.getBoolean(options,"androidLockWorkaround",false);
